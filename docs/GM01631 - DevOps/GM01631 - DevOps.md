...
the Codecademy DevOps course. I do not take ownership of the material
...
operation team create and maintains the infrastructure that the
...
Let us next discuss how DevOps seeks to always keep improving.
...
Let us discuss the way DevOps creates these feedback loops.

#### Feedback Loops

DevOps employs a variety of strategies to incorporate feedback into its
processes on an ongoing basis. Let us look at a few of these feedback
loops.
...
Adding metrics to the software can be extremely helpful, but having too
many can produce unwanted noise. We must focus on metrics that affect
the customer. Some of these include:
...
routers, switches, data centres, and cables. Software components include
...
![A diagram of different colored arrows Description automatically generated](media/image3.png)
...
strategies can differ. Some examples of deployment strategies include:
...
systems each need some slice of the host machine's resources. Let us
look at how a successor of virtualization solved this problem. This
successor is containerization.
...
outweigh the disadvantages. Most companies today use cloud-based
services. The biggest providers are Amazon Web Services (AWS), Microsoft
...
In time, some of the downsides may get worked out. Serverless is still
new and catching on fast. It was not popularized until 2014 with the
introduction of AWS Lambda. Microsoft Azure Functions and Google Cloud
Functions followed shortly after.
...
1. Next, they configure the IP address, hostname, security system, and
    DNS settings.
...
Both approaches can achieve the same configuration. The difference is
that the declarative approach focuses on what infrastructure state you
want to achieve, while the imperative approach focuses on how to get
there.
...
- **Speedy Initial Development --** Starting to write a monolithic
  ...
  framework with which they are comfortable. It is possible to get a
  basic application up and running in minutes.
...
- **Simple Testing --** Like deploying a monolith, testing a monolith
  ...
  be configured to test the application.
...
- **Complex Codebase --** As a monolith grows, its codebase becomes
  ...
  other features. If developers are not aware of these dependencies,
  they can introduce unexpected bugs.
...
- **Slower Initial Development -** Getting an application up and running
  is not as simple as with a monolith. A microservice architecture
  requires creating and deploying many small services whose interactions
  can become complex.
...
The ability to see inside a system leads to more informed solutions. But
not individuals resolve all issues.
...
- **Response Time --** When requests take a long time to resolve, that
  is usually a sign something is wrong in our system.
...
- **Unactionable Alerts --** This type of alert has little to do with a
  problem and does not need anything done. Like false negatives, we
...
how it might fail and design the system to respond.
...
external services. We cannot control whether a service or API we use
will stop being supported or be shut down.
...
same results repeatedly. Caching stores the commonly requested results,
reducing the work necessary to resolve similar requests. Caching
separates requests into two types:
...
Adding or removing servers is not enough. We need a system to direct the
appropriate amount of traffic to any servers we have. Let us discuss the
...
differ from reality. While this kind of analysis can produce an
estimate, we should not rely on it for exact amounts.
...
to see how our system responds. Let us look at some ways engineers test
...
- **Less error-prone --** automation can perform a task more
  consistently than a person.

- **Cheaper --** workers do not have to be paid to do these repetitive
  workflows.

#### What Can We Automate?

We can integrate automation into every aspect of software development.
Let us look at some of the ways automation can play a role in software
development:
...
with bigger projects there are long review periods for larger feature
branches and there could be many conflicts when merging large branches
...
the solution due to the slow cycle of merges and larger branch sizes.
...
- **Dark launching --** like feature flags, but certain users have
...
Let us walk through the full CI/CD process. Keep in mind that CI and CD
processes are automated:

1. A developer makes a change and commits their code.

1. CI merges the change.
...
CD capabilities. If CI is set up for a project, the same tool can be
used when setting up the CD servers.
